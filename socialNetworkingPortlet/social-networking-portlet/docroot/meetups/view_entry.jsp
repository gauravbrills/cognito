...
<link
	href="<%=request.getContextPath()%>/meetups/css.jsp?themeId=<%=themeDisplay.getTheme().getThemeId()%>&amp;colorSchemeId=<%=themeDisplay.getColorScheme().getColorSchemeId()%>&amp;t=<%=portlet.getTimestamp()%>"
...
<%
	...
	try {
		meetupsEntry = MeetupsEntryLocalServiceUtil
				.getMeetupsEntry(meetupsEntryId);
	} catch (NoSuchMeetupsEntryException nsmee) {
%>

<span class="portlet-msg-error"> The meetup could not be found. </span>
...
<%
	...
	try {
		meetupsRegistration = MeetupsRegistrationLocalServiceUtil
				.getMeetupsRegistration(themeDisplay.getUserId(),
						meetupsEntryId);
	...
	portletURL.setParameter("meetupsEntryId",
...
<%
	int yesTotal = MeetupsRegistrationLocalServiceUtil
			.getMeetupsRegistrationsCount(meetupsEntryId,
...
<c:choose>
	<c:when test="<%= themeDisplay.isSignedIn() %>">
		<form action="<portlet:actionURL name="updateMeetupsRegistration" />"
			method="post" name="<portlet:namespace />fm"
			onSubmit="<portlet:namespace />updateMeetupsRegistration(); return false;">
			<input name="<portlet:namespace />redirect" type="hidden"
				value="<%=currentURL%>" /> <input
				name="<portlet:namespace />meetupsEntryId" type="hidden"
				value="<%=meetupsEntryId%>" />

			<liferay-ui:message key="will-you-attend" />

			<input
				<%=(status == MeetupsConstants.STATUS_YES) ? "checked"
							: ""%>
				name="<portlet:namespace />status" type="radio"
				value="<%=MeetupsConstants.STATUS_YES%>" />
			<liferay-ui:message key="yes" />

			<input <%=(status == MeetupsConstants.STATUS_NO) ? "checked"
							: ""%>
				name="<portlet:namespace />status" type="radio"
				value="<%=MeetupsConstants.STATUS_NO%>" />
			<liferay-ui:message key="no" />

			<input
				<%=(status == MeetupsConstants.STATUS_MAYBE) ? "checked"
							: ""%>
				name="<portlet:namespace />status" type="radio"
				value="<%=MeetupsConstants.STATUS_MAYBE%>" />
			<liferay-ui:message key="maybe" />

			<br />
			<br />

			<liferay-ui:input-field model="<%= MeetupsRegistration.class %>"
				bean="<%= meetupsRegistration %>" field="comments" />

			<br />
			<br /> <input type="submit"
				value="<liferay-ui:message key="register" />" />

		</form>

		<br />

		<liferay-ui:tabs names="attending,not-attending,maybe-attending"
		...
		<%
			int tabs1Status = MeetupsConstants.STATUS_YES;
					...
					SearchContainer searchContainer = new SearchContainer(
							...
							SearchContainer.DEFAULT_CUR_PARAM, 10, portletURL,
							null, null);

					int total = MeetupsRegistrationLocalServiceUtil
							.getMeetupsRegistrationsCount(meetupsEntryId,
									tabs1Status);

					searchContainer.setTotal(total);

					List<MeetupsRegistration> results = MeetupsRegistrationLocalServiceUtil
							.getMeetupsRegistrations(meetupsEntryId,
									tabs1Status, searchContainer.getStart(),
		...
		<div class="response">
			...
			<c:if
				test="<%= Validator.isNotNull(curMeetupsRegistration.getComments()) %>">
				<div class="comments">
	...
	<c:otherwise>
		<div>
			You have to be signed in to register for this meetup. Please <a
				href="<%=themeDisplay.getURLSignIn()%>">sign in</a> or <a
				href="<%=themeDisplay.getURLCreateAccount()%>">create an
